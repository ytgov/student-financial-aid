<template>
  <v-card color="#eee5d1" elevation="0">
    <v-card-text>
      ...
      <v-form>
        <div v-for="(item, key) in this.application.draft.csfa_accomodation.accomodations">
          <v-row>
            <v-col cols="12">
              <h3>{{ item.heading }}</h3>
            ...
            <v-col cols="12" md="4" v-if="['Living at Parents', 'Both'].includes(item.living)">
              <v-text-field
                type="number"
                variant="outlined"
                bg-color="white"
            ...
            <v-col cols="12" md="6" v-if="['Living on Own', 'Both'].includes(item.living)">
